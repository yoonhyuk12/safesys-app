// 장비 일일점검 대장 SQL 테스트용 PGlite 인메모리 DB 준비와 로그인 사용자 전환 헬퍼.
...
export const ANSWERS = [
  ...
  { id: 'equipment-01-02', category: '작동검사', text: '경보음이 정상 작동하는가?', result: 'fail', note: '경보음 미작동' },
]
...
export function insertInspection(db, overrides = {}) {
  ...
  return db.query(
    `INSERT INTO public.equipment_daily_inspections
       (project_id, equipment_type, equipment_name, inspection_date, inspector_name, signature, answers, created_by)
     VALUES ($1::uuid, $2, $3, $4::date, $5, $6, $7::jsonb, $8::uuid)
     RETURNING id`,
    [row.project_id, row.equipment_type, row.equipment_name, row.inspection_date,
      row.inspector_name, row.signature, row.answers, row.created_by]
  )
}

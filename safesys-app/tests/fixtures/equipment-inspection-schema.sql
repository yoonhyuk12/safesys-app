-- 장비 일일점검 대장 RLS 회귀 테스트용 최소 스키마.
-- Supabase의 auth.uid()와 projects 조회 RLS를 PGlite에 재현해, 마이그레이션의 정책을 그대로 실행해 본다.
...
-- 운영 정책(프로젝트_조회_통합정책 + projects_select_shared)의 요지를 옮긴 것이다.
-- 소유자, 공유받은 사용자, 관할 본부·지사의 발주청만 프로젝트를 본다.
...
GRANT SELECT ON public.user_profiles, public.project_shares TO authenticated;

// 남이 올린 사고보고까지 관리할 수 있는 사용자인지 소속과 현장 관할로 판정하는 순수 모듈
...
/**
 * 본인이 올리지 않은 사고보고까지 삭제할 수 있는지 판정한다.
 * 수정은 이 판정을 쓰지 않는다 — 그 현장을 열 수 있는 사용자면 누구나 고친다.
 * 전사 범위 사용자는 어느 현장이든 되고, 본부 소속은 자기 본부가 관할하는 현장일 때만 된다.
 * 작성자 본인 여부는 여기서 보지 않는다 — 호출하는 쪽이 `created_by`와 함께 OR로 묶는다.
 * `database/20260718-0506_add_project_accidents.sql`의 삭제 RLS와 같은 갈래이며,

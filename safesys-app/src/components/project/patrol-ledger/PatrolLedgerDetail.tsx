...
export default function PatrolLedgerDetail({ record, canEdit, canDelete, busy, downloading, onBack, onEdit, onDelete, onDownload }: {
  ...
  return <div className="w-full max-w-4xl mx-auto min-w-0 bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    <div className="bg-blue-600 text-white px-4 py-3"><h2 className="font-semibold text-sm sm:text-base break-words">작업장 순회 점검표{record.contractor_name && `(${record.contractor_name})`}</h2></div>
    ...
    <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      <button disabled={busy} onClick={onDownload} className="min-h-[44px] px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed inline-flex items-center gap-2"><Download className="h-4 w-4" />HWPX 다운로드</button>
      {canEdit && <button disabled={busy} onClick={onEdit} className="min-h-[44px] px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors disabled:opacity-50 inline-flex items-center gap-2"><Pencil className="h-4 w-4" />수정</button>}
      {canDelete && <button disabled={busy} onClick={onDelete} className="min-h-[44px] px-4 py-2 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition-colors disabled:opacity-50 inline-flex items-center gap-2"><Trash2 className="h-4 w-4" />삭제</button>}
      </div>
    </div>
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="grid sm:grid-cols-2 gap-4 p-4 border-t border-gray-200">
        <div className="min-w-0 bg-white rounded-lg shadow-sm border border-gray-200 p-3"><h3 className="text-xs text-gray-500 mb-1">{PATROL_LEDGER_PHOTO_KIND_LABELS[record.finding_photo_kind] ?? '지적사진'}</h3>
      ...
      {record.finding_photo_kind === 'finding' && record.finding_text.trim() && <div className="grid sm:grid-cols-2 gap-4 px-4 pb-4">
        ...
        <div className="min-w-0 bg-white rounded-lg shadow-sm border border-gray-200 p-3"><h3 className="text-xs text-gray-500 mb-1">조치내용{record.action_date ? ` (${record.action_date})` : ''}</h3><p className="text-sm text-gray-600 whitespace-pre-wrap break-words">{record.action_text || '없음'}</p></div>
      </div>}
      <dl className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-3 sm:p-4">
        {[
        ...
        ].map(([label, value]) => <div key={label} className="min-w-0"><dt className="text-xs text-gray-500">{label}</dt><dd className="text-sm text-gray-900 break-words">{value || '—'}</dd></div>)}
        <div className="min-w-0"><dt className="text-xs text-gray-500">서명</dt><dd>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src={record.signature} alt="점검자 서명" className="h-16 max-w-full object-contain border border-gray-200 rounded-md bg-white" />
        </dd></div>
      </dl>
    </div>
    </div>

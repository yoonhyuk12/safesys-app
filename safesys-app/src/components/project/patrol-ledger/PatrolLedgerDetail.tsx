...
export default function PatrolLedgerDetail({ record, canEdit, canDelete, busy, downloading, onEdit, onDelete, onDownload }: {
  ...
  return <div className="space-y-4">
    ...
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
      <h2 className="p-4 text-lg font-semibold text-gray-900 text-center">작업장 순회 점검표{record.contractor_name && `(${record.contractor_name})`}</h2>
      <table className="w-full">
        <thead className="bg-gray-50 border-b border-gray-200"><tr>{['No.', '점검사항', '점검결과'].map(label => <th key={label} className="px-3 py-3 text-center text-xs font-medium text-gray-500">{label}</th>)}</tr></thead>
        <tbody className="bg-white divide-y divide-gray-200">{Array.from({ length: 10 }, (_, index) => {
          const item = record.items[index]
          return <tr key={index}>
            <td className="px-3 py-3 text-sm text-center">{item?.no ?? index + 1}</td>
            <td className="px-3 py-3 text-sm text-left text-gray-900">{item ? `(${item.category}) ${item.text}` : '—'}</td>
            <td className="px-3 py-3 text-sm text-center"><span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-medium whitespace-nowrap ${item?.result === '미흡' ? 'bg-red-100 text-red-800' : item?.result === '양호' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{item?.result || '미점검'}</span></td>
          </tr>
        })}</tbody>
      </table>
      <div className="grid sm:grid-cols-2 gap-4 p-4 border-t border-gray-200">
        <div><h3 className="text-sm font-medium text-gray-700 mb-2">지적사진 (없는 경우 점검사진)</h3>
        ...
        <div><h3 className="text-sm font-medium text-gray-700 mb-2">지적사항</h3><p className="text-sm text-gray-600 whitespace-pre-wrap break-words">{record.finding_text || '없음'}</p></div>
      </div>
      <dl className="grid grid-cols-2 sm:grid-cols-3 gap-4 border-t border-gray-200 p-4">
        {[
        ...
        ].map(([label, value]) => <div key={label}><dt className="text-xs text-gray-500">{label}</dt><dd className="text-sm text-gray-900">{value || '—'}</dd></div>)}
        <div><dt className="text-xs text-gray-500">서명</dt><dd>
          {/* eslint-disable-next-line @next/next/no-img-element */}

...
import { AlertTriangle, Calendar, ArrowLeft, Download } from 'lucide-react'
import { type HeadquartersInspection, type Project } from '@/lib/projects'
import { type UserProfile } from '@/lib/supabase'
import { HEADQUARTERS_OPTIONS, BRANCH_OPTIONS } from '@/lib/constants'
import LoadingSpinner from '@/components/ui/LoadingSpinner'

interface HeadquartersInspectionStatusProps {
  ...
  onHeadquartersInspectionClick: (inspection: HeadquartersInspection) => void
  onBackClick: () => void
  onDownloadClick?: () => void
}

const HeadquartersInspectionStatus: React.FC<HeadquartersInspectionStatusProps> = ({
  userProfile,
  projects,
  selectedQuarter,
  setSelectedQuarter,
  ...
  onHeadquartersInspectionClick,
  onBackClick,
  onDownloadClick
}) => {
  // 본부 불시점검 레코드에서 파생 상태 계산
  const deriveInspectionState = (inspection: HeadquartersInspection): 'completed' | 'pending' => {
    if (inspection.issue1_status !== 'completed') return 'pending'
    if (inspection.issue2_status && inspection.issue2_status !== 'completed') return 'pending'
    return 'completed'
  }
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="inline-flex items-center rounded-md border border-gray-300 bg-white px-3 py-1 text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 disabled:opacity-50"
              aria-label="본부 불시점검 보고서 다운로드"
            >
        ...
        {inspectionDataLoading ? (
          ...
          if (selectedSafetyBranch) {
            ...
            const projectInspectionCounts = branchProjects.map(project => {
              const projectInspections = headquartersInspections.filter(
                inspection => inspection.project_id === project.id && deriveInspectionState(inspection) === 'completed'
              )
              const pendingInspections = headquartersInspections.filter(
                inspection => inspection.project_id === project.id && deriveInspectionState(inspection) === 'pending'
              )
              // 해당 분기 공사중 여부
              ...
              // 디버깅: is_active 값 출력
              console.log(`프로젝트 ${project.project_name}:`, 'is_active=', ia, 'typeof=', typeof ia)
              
              if (ia && typeof ia === 'object') {
                const key = `q${quarterNum}` as 'q1' | 'q2' | 'q3' | 'q4'
                // 선택 분기 값만 기준: true면 대상
                isTarget = !!ia[key]
                console.log(`  - 분기 ${key}: ${ia[key]} → isTarget: ${isTarget}`)
              } else {
                // 구형 boolean은 분기판별 불가 → 대상 아님
                isTarget = false
                console.log(`  - 구형 boolean/기타: ${ia} → isTarget: false`)
              }
              return {
                ...project,
            ...
            return (
              <div>
                ...
                <div className="bg-orange-50 rounded-lg p-4 mb-6">
                  ...
                  <p className="text-orange-700">총 프로젝트 수: {branchProjects.length}개</p>
                </div>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">완료된 점검</th>
                          <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">대기 중인 점검</th>
                          ...
                          <tr key={project.id || index} className="hover:bg-gray-50 divide-x divide-gray-200">
                            ...
                            <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap border-r border-gray-200">
                              <div className="text-xs sm:text-sm font-semibold text-orange-600">
                                {project.pendingCount}건
                              </div>
                            </td>
                            <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap">
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                project.pendingCount > 0 ? 'bg-yellow-100 text-yellow-800' :
                                project.inspectionCount > 0 ? 'bg-green-100 text-green-800' :
                                'bg-gray-100 text-gray-800'
                              }`}>
                                {project.pendingCount > 0 ? '점검대기' :
                                 project.inspectionCount > 0 ? '점검완료' : '점검없음'}
                              </span>
                            </td>
                          </tr>
                        ))}
          ...
          // 본부 단위에서는 지사별 점검 통계
            ...
            // 지사별로 점검 횟수 및 점검대상 수 집계 (선택 분기 기준)
            ...
            headquartersInspections.forEach((inspection: HeadquartersInspection) => {
              ...
              if (branchStats.has(branch)) {
                const entry = branchStats.get(branch)!
                const state = deriveInspectionState(inspection)
                if (state === 'completed') {
            ...
            return branchStats.size === 0 ? (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">지사명</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">총 프로젝트 수</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">점검대상 수</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">점검횟수(대상)</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">대기 건수</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">점검률(대상)</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(() => {
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">프로젝트명</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">점검일</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">점검자</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-200">종합평가</th>
                      <th className="px-3 py-2 sm:px-6 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">상태</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {headquartersInspections.map((inspection: HeadquartersInspection, index: number) => (
                      <tr 
                        key={inspection.id || index}
                        className="hover:bg-gray-50 cursor-pointer transition-colors divide-x divide-gray-200"
                        onClick={() => onHeadquartersInspectionClick(inspection)}
                      >
                        <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap border-r border-gray-200">
                          <div className="text-xs sm:text-sm font-medium text-gray-900">
                        ...
                        <td className="px-3 py-2 sm:px-6 sm:py-4 whitespace-nowrap">
                          {(() => {
                            ...
                            return (
                              <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                                state === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'
                              }`}>
                                {state === 'pending' ? '대기중' : '완료'}
                              </span>
                            )
                          })()}
                        </td>
...
export default HeadquartersInspectionStatus
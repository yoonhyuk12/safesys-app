...
const SafetyInspectionLedgerView: React.FC<SafetyInspectionLedgerViewProps> = ({
  ...
  const handleBulkWordDownload = async (level: 'hq' | 'branch' | 'project') => {
    ...
    try {
      ...
      // 프로젝트 테이블(프로젝트명 컬럼)과 동일한 순서로 정렬
      // UI: inspection_count desc. 동수는 프로젝트명(가나다), 같은 프로젝트는 점검일 오름차순
      const projectInfo = new Map<string, { count: number; name: string }>(
        targetProjects.map(p => {
          const ic: any = inspectionCounts.find(c => c.project_id === p.id)
          return [p.id, {
            count: ic?.inspection_count || 0,
            name: p.project_name || '',
          }]
        })
      )

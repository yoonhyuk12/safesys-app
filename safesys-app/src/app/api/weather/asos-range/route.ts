...
function skyFromCloud(cloudAvg: number | null): WeatherData["sky"] {
...
declare global {
  ...
  var weatherRangeCache: Map<string, { data: RangeResponse; timestamp: number }> | undefined;
}
...
function splitDateRange(start: string, end: string): { tm1: string; tm2: string }[] {
  ...
  while (current <= endDate) {
    const rangeStart = current.toISOString().slice(0, 10).replace(/-/g, '');
    current.setDate(current.getDate() + 30); // 31일 단위
    if (current > endDate) current = new Date(endDate);
    const rangeEnd = current.toISOString().slice(0, 10).replace(/-/g, '');
    ranges.push({ tm1: rangeStart, tm2: rangeEnd });
...
/**
 * 기간 조회 API
 * GET /api/weather/asos-range?lat=37.39&lon=127.23&start=20260101&end=20260131
 *
 * 응답: { stnId, stnName, data: [{ date, sky, tempAvgC, rainSumMm, cloudAvg, summary }, ...] }
 */
export async function GET(req: Request) {
  ...
  if (!global.weatherRangeCache) {
    global.weatherRangeCache = new Map();
  }
  const cacheKey = `${lat.toFixed(2)}_${lon.toFixed(2)}_${start}_${end}`;
  const cached = global.weatherRangeCache.get(cacheKey);
  ...
  try {
    ...
    for (const range of ranges) {
      const url = `https://apihub.kma.go.kr/api/typ01/url/kma_sfcdd3.php?tm1=${range.tm1}&tm2=${range.tm2}&stn=${station.stnId}&help=0&mode=0&authKey=${encodeURIComponent(getKmaHubKey())}`;
      const text = await fetchText(url);
      const dataMap = parseDailyData(text);
      dataMap.forEach((v, k) => allData.set(k, v));
    }

    // 3. 결과 배열 생성
    const data: WeatherData[] = [];
    const currentDate = new Date(`${start.slice(0, 4)}-${start.slice(4, 6)}-${start.slice(6, 8)}`);
    const endDate = new Date(`${end.slice(0, 4)}-${end.slice(4, 6)}-${end.slice(6, 8)}`);
    ...
    // 캐시 저장
    global.weatherRangeCache.set(cacheKey, { data: result, timestamp: Date.now() });
    return NextResponse.json(result);

  } catch (e: any) {
    console.error("날씨 기간조회 API 오류:", e.message);

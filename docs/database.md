...
- `project_accidents` — 프로젝트별 사고 이력과 피해·예방조치 정보. 미등록 현장은 `project_id` NULL + `external_project_name`·본부·지사 직접입력. 등록 현장 사고는 그 현장을 볼 수 있는 로그인 사용자(시공사·감리단·관할 발주청)가 작성하고 작성자 본인만 고치거나 지운다. 조회 RLS는 `projects` 조회 정책에 얹어 관할을 판정하며, 안전캐비넷 A(조치) 사고보고와 안전대시보드 사고현황이 같은 테이블을 쓴다. 적용 순서는 `database/20260718-0506_add_project_accidents.sql` → `database/20260718-0830_project_accidents_external_site.sql` → `database/20260831-1730_project_accidents_workers_comp_claim.sql` → `database/20260916-1032_사고보고_현장작성_권한.sql`이다. 권한 회귀는 `safesys-app/tests/project-accidents-sql.test.mjs`가 실제 마이그레이션을 PGlite에 얹어 검증한다

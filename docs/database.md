...
- `patrol_ledger_inspections` — AI 순회점검대장. TBM 또는 직접 입력 작업내용과 항목 1~13건(앞 10건 AI, 뒤 3건 TBM 대책 이행 여부)(`items` JSONB), 양호·미흡 결과, 지적사항·사진, 점검자 개인 PNG 서명을 저장한다. 조회·등록은 현장 조회 RLS를 따르고 수정은 작성자 본인에게 내용 열만 허용하며 삭제는 작성자·현장 소유자·관할 발주청에게 허용한다. 적용 순서는 `database/20260917-1700_순회점검대장.sql` → `database/20260917-1701_merge_projects_patrol_ledger.sql` → `database/20260917-1800_순회점검_사진구분.sql` → `database/20260917-1900_순회점검_주간테마.sql` → `database/20260917-2000_순회점검_조치사항.sql` → `database/20260921-1000_순회점검_해당없음.sql` → `database/20260922-1100_순회점검_TBM대책행.sql` → `database/20260923-1000_순회점검_기타사항.sql`이며, 병합 시 기록을 이전하고 프로젝트 삭제 시 지적·조치 사진 URL도 수집한다. 조치 칸(`action_text`·`action_photo_url`·`action_date`)은 지적사항 관리대장에서 현장을 볼 수 있는 누구나 고칠 수 있고, 그 외 점검 내용은 트리거 `patrol_ledger_guard_non_author_update`가 작성자 본인에게만 허용한다. `finding_photo_kind`(finding=지적사진, overview=전경사진)는 CHECK로 전경사진일 때 `finding_text`가 비어 있도록 강제한다. 전경사진의 `other_text`는 기타사항으로 저장하고 지적 건수에 포함하지 않는다.
- `patrol_ledger_weekly_themes` — 회사 공통 주간 점검 테마. 본부급 발주청만 쓸 수 있고 모든 로그인 사용자가 읽는다.
...
- `project_accidents` — 프로젝트별 사고 이력과 피해·예방조치 정보. 미등록 현장은 `project_id` NULL + `external_project_name`·본부·지사 직접입력. 등록 현장 사고는 그 현장을 볼 수 있는 로그인 사용자(시공사·감리단·관할 발주청)가 작성하고 작성자와 무관하게 함께 고치되(`created_by`는 트리거로 불변), 지우는 것은 작성자 본인 또는 관할 본부급 이상만 한다. 조회·수정 RLS는 `projects` 조회 정책에 얹어 관할을 판정하며, 안전캐비넷 A(조치) 사고보고와 안전대시보드 사고현황이 같은 테이블을 쓴다. 적용 순서는 `database/20260718-0506_add_project_accidents.sql` → `database/20260718-0830_project_accidents_external_site.sql` → `database/20260831-1730_project_accidents_workers_comp_claim.sql` → `database/20260916-1032_사고보고_현장작성_권한.sql` → `database/20260916-1418_사고보고_보고서_항목.sql` → `database/20260916-1720_사고보고_산재요양_예상일수.sql` → `database/20260917-1555_사고보고_산재신청_연도.sql`이다. 산재신청 연도(`workers_comp_claim_year`, 2000~2100 CHECK)는 컬럼 도입 시 기존 행을 모두 2026으로 채웠고 새 등록은 앱이 당해 연도를 기본값으로 넣는다. 권한 회귀는 `safesys-app/tests/project-accidents-sql.test.mjs`가 실제 마이그레이션을 PGlite에 얹어 검증한다
- **알려진 결함(후속 과제).** `prevent_project_accidents_created_by_change` 트리거는 가드 없이 모든 UPDATE에 걸려 있어, `auth.users` 삭제 시 FK의 `ON DELETE SET NULL`까지 `project_accidents.created_by는 변경할 수 없습니다`로 되돌린다. 사고 기록이 있는 계정은 가입 해지가 통째로 실패한다. 해법은 `database/20260914-1339_quality_preserve_created_by.sql`이 쓰는 `current_user = 'authenticated' AND pg_trigger_depth() = 1` 가드이며, 재현은 위 테스트의 `test.todo` 항목이다
- **사고발생보고서 추가 항목.** `database/20260916-1418_사고보고_보고서_항목.sql`은 위 사고보고 권한 SQL 다음에 적용한다. `project_accidents.report_details` JSONB에 보고일·보고자·피해자·신고·조치·연락처·사진을 저장한다. 사진은 JPEG data URL 최대 2장과 각 사진 설명이며 CHECK 제약으로 형태·길이를 검사한다. 기존 행은 NULL이고, 대시보드 간단 수정에서 이 컬럼을 생략하면 기존 값이 보존된다. 목록·통계는 사진을 제외한 명시적 컬럼만 읽고 보고서 상세·편집·출력에서 추가 데이터를 조회한다. 새 자식 테이블이나 Storage 파일이 없어 기존 프로젝트 삭제·병합과 함께 처리된다.
- **산재요양 예상 일수.** 위 1418 SQL 다음에 `database/20260916-1720_사고보고_산재요양_예상일수.sql`을 적용한다. `report_details.expectedTreatmentDays`는 선택 키이며 빈 문자열(미입력) 또는 0 이상 9,007,199,254,740,991 이하 정수의 십진 숫자 문자열(최대 200자)만 받는다. JSON 숫자·null·음수·소수·비숫자는 거부한다. 기존 키 없는 보고서와 NULL은 그대로 허용하고 `lost_workdays`와 복사·계산하지 않는다. 검증함수만 확장하며 기존 사진·문자·배열 검사와 RLS를 보존한다.
- `workers` — 작업자 프로필/등록
...
- 2026-09-17 순회점검대장 마이그레이션 적용 기준 `projects`를 참조하는 자식 테이블은 29개이며, 비용 귀속용 `ai_usage_logs`(`ON DELETE SET NULL`)만 예외이고 나머지 28개는 CASCADE다. `project_accidents`도 이 규칙을 따른다. 새 기능의 테이블도 빠짐없이 이 패턴을 따라야 한다. 감사는 `pg_constraint`에서 `confrelid = 'projects'`인 FK의 `confdeltype = 'c'`(=CASCADE) 여부로 확인한다.
...
- **프로젝트 병합(`merge_projects` DB 함수)도 함께 갱신한다.** 병합은 자식 테이블의 `project_id`를 target으로 UPDATE한 뒤 source를 삭제하므로, 함수의 UPDATE 목록에 없는 자식 테이블은 CASCADE로 유실된다. 함수는 실제 FK 테이블 수와 자신이 아는 개수(현재 29)가 다르면 예외로 중단하도록 되어 있으니, **새 자식 테이블 추가 시 UPDATE 목록과 개수 가드를 함께 갱신**해야 병합이 다시 동작한다.
...
- 병합 SQL 회귀 테스트는 PGlite 인메모리 Postgres에서 실제 마이그레이션 파일을 적용해 돌린다 — `cd safesys-app && npm run test:merge-sql`. 스키마 픽스처는 `safesys-app/tests/fixtures/merge-projects-schema.sql`이며, 적용할 마이그레이션 목록은 `merge-projects-db.mjs`의 `MIGRATION_PATHS`다. 자식 테이블을 늘렸으면 픽스처·`CHILD_TABLES`·개수 가드를 함께 고친다.
- 새 테이블의 RLS도 같은 방식으로 검증할 수 있다 — `npm run test:equipment-sql`은 `auth.uid()`와 `projects` 조회 정책을 PGlite에 재현한 뒤 실제 마이그레이션을 적용해 관할·작성자·무결성 CHECK를 확인한다.

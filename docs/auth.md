...
- 성과총괄표 반려 통보와 취소는 서버에서 발주청 역할을 확인한다. 취소 버튼은 통보자 본인만 활성화하며 다른 사용자에게도 비활성 상태로 표시한다. 취소 API는 UPDATE 조건에 `rejected_by = 인증 사용자 ID`를 포함한다. 취소는 반려 사유·통보자·통보일시·읽음 정보를 초기화하고 현재 서명과 문서 내용은 유지한다.

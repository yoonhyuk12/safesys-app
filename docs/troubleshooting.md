...
실시대장의 `CSI 성적서` → `자체 품질시험 (CSI 로그인)`에서 로그인한 뒤 CSI 사업을 선택하고 실적을 조회한다. 시료봉인 가져오기는 자체 품질시험으로 교체되었고, 발급 성적서 검색 탭은 유지한다.

- 자체 품질시험 목록은 `bizMngNo`가 없으면 빈 목록이 돌아온다. 사업별 현황에서 읽은 사업번호를 목록·상세 요청에 전달해야 한다.
- 등록일 대신 상세의 시험완료일을 가져온다. 시험항목·기준·여러 측정값과 적합/부적합 판정을 보존하고, 실시대장 판정인 합격/불합격으로 변환한다.
- 가져온 내용은 입력 폼에만 채운다. 공종과 내용을 확인하고 저장한다. 서명은 자동으로 복사하지 않는다.
- 사업명·실적 검색 입력란은 제공하지 않으며, 사업 선택 후 조회한 실적 전체를 표시한다. 사업 목록이나 실적이 제한 시간·페이지 상한으로 일부만 조회되면 안내를 표시한다.
- 실적 조회 위의 조회 기간은 CSI 등록일 기준이며 기본값은 오늘까지 최근 1달이다. 1달·2달·3달·6달·1년·2년 프리셋으로 기간을 지정하거나 시작일·종료일을 직접 입력한 뒤 실적 조회를 누른다. CSI에 전달하는 `startYmd`·`endYmd`는 하이픈 없는 `YYYYMMDD` 형식이어야 한다.
...
## 프로젝트 나라장터 갱신 후 착공일이 당해년도로 바뀔 때 (장기계속 연차 계약)

프로젝트 상세의 나라장터 갱신(`handleG2bSync`)은 `/api/g2b/contract?no=<확정계약번호>&latest=1`로 최신 차수 계약을 받아 갱신한다. **장기계속(연차) 계약은 해마다 확정계약번호가 새로 부여되므로**(R25TA… → R26TA…) 이때 오는 `startDate`는 최초 계약의 착공일이 아니라 **당해년도 차수의 시작일**이다. 실측: 확정계약번호 `R26TA0137792601` 조회는 2026-01-26 한 건만 주지만, 공고번호 `2023033863100`으로 조회하면 최초 계약(2023-05-09)을 포함한 4건이 온다 (2026-09-09 확정). 준공일(총준공일)·금액·업체는 최신 차수 값이 맞고 착공일만 틀렸다.

- **해결**: 최신 차수 조회와 함께 `g2b_ntce_no`(공고번호)로 `latest` 없이 한 번 더 조회해 두 결과를 합치고, `src/lib/g2b-contract-period.ts`의 `earliestStartDate()`로 가장 이른 착공일을 고른다. 공고번호 조회는 best-effort — 실패해도 갱신을 막지 않는다.
- **주의**: `g2b_ntce_no`가 비어 있거나 확정계약번호와 같은 프로젝트는 보조 조회를 할 수 없어 여전히 최신 차수 시작일이 저장된다. 착공일이 이상하면 프로젝트 수정에서 **공고번호(`g2b_ntce_no`)를 채운 뒤** 다시 갱신한다.

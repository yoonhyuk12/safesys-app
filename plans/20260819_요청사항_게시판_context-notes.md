...
게시판 초안은 `bg-gray-50` 밝은 배경이라 대시보드(`bg-gradient-to-b from-blue-950 via-blue-900 to-slate-900` + 흰색 헤더 바)와 이질적이었다. 페이지 셸을 대시보드와 같은 남색 그라데이션으로 바꾸고, `BoardHeader` 를 대시보드 헤더와 동일한 `bg-white shadow-sm border-b` · 높이 `h-16` · 파란 아이콘 + 제목 구성으로 맞췄다.

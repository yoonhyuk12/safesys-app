...
- 2026-07-20 사용자 디자인 피드백 2. 감독 일괄서명 모달을 조회 전용으로 전환 — BulkSignModal에 VIEW_ONLY_SIGNERS(['supervisor']) 도입. 닫기 외 조작(체크박스·전체선택·새로고침·서명하기) 시 가운데 "현재는 조회만 가능합니다" 토스트 1.8초 표시. 시공사(contractor) 모달은 기존대로 서명 가능. 되돌리려면 VIEW_ONLY_SIGNERS를 빈 배열로.

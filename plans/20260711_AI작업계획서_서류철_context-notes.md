...
## 2026-07-11 Phase 4 PDF 배선·원본 대조

- **목록·완료 화면 배선** — 목록 행과 마법사 저장 완료 화면에서 `plan_type`별 4종 PDF 빌더를 호출한다. 생성 중에는 다른 다운로드를 막고 스피너를 표시하며, 실패 메시지를 화면에 남긴다. 저장 직후 목록 상태만 갱신해 완료 화면을 유지하고 사용자가 `목록으로`를 눌러 닫는다.
- **원본 페이지 대응** — 붙임2-1은 표지를 제외한 원본 2~4쪽과 빌더 1~3쪽, 붙임2-4도 원본 2~4쪽과 빌더 1~3쪽이 대응한다. 붙임2-2는 원본 2~4쪽의 사전조사 유형 중 선택한 유형만 빌더 1쪽에 출력하고, 원본 5쪽과 6쪽을 빌더 2쪽과 3쪽에 출력한다.
- **전기 양식 대응** — 붙임2-3 원본은 가로 물리 2쪽에 논리 4쪽을 2-up으로 배치했다. 표지를 제외하고 본표, 작업순서·체크리스트·인계인수, 도면·사진을 A4 세로 빌더 1~3쪽으로 분리했다. 도면·사진 업로드가 없으면 원본의 예시 사진 4칸을 대신할 사용자 데이터가 없으므로 세 번째 쪽은 생략한다.
- **대조 조정** — 150dpi로 원본 16개 물리 페이지를 렌더해 시각 대조했다. 확인된 차이에 한해 결재 부제를 `(수급업체용)`으로 수정하고, 범례의 출입통제구역을 보라 점선 타원, 안전표지판을 녹색 채움 사각으로 맞췄다. 지연입력 값이 공란이어도 원본처럼 `ton`, `m`, `kg` 단위는 남긴다. 건설기계·전기 빌더는 대조 결과 수정하지 않았다.
- **검증 결과** — 목록·마법사 배선과 PDF 조정 파일 대상 엄격 ESLint, `npx tsc --noEmit`, `git diff --check`가 종료 코드 0으로 통과했다.

## 2026-07-11 지도 고정 빈 캡처 수정·드로잉 단계 레이아웃 재구성

- **빈 캡처 원인** — 카카오맵이 래스터 타일이 아닌 벡터 SVG 레이어(사용자 페이지 피드백 기준 7345×4125px `<svg>`)로 렌더링되어 html2canvas가 흰 이미지를 반환했고, 에러 없이 흰 배경으로 고정돼 "지도가 사라지는" 증상이 났다.
- **캡처 3단계 폴백** — ① 지도 DOM 복제 후 `img`/SVG `image` 원격 소스를 `/api/map-tile` 프록시로 받아 data URL 인라인(SVG-이미지 렌더링은 외부 요청이 차단되므로 필수) → foreignObject SVG 직렬화 → 캔버스 래스터화. ② html2canvas 직접. ③ html2canvas + 프록시. 각 결과는 `isMostlyBlankCanvas`(픽셀 샘플링, 편차 2% 미만이면 빈 이미지, 오염 캔버스 포함)로 검증하고, 전부 실패하면 고정하지 않고 지도를 유지한 채 에러를 안내한다. Safari의 foreignObject 캔버스 오염은 toDataURL 실패 → 다음 폴백으로 흡수된다.
- **레이아웃** — 사용자 지시대로 데스크탑(lg 이상)은 지도 좌측 + 우측 15rem 도구 패널(배지형 버튼), 모바일은 지도를 가운데 두고 위(배경 유형 또는 도구 배지)·아래(고정/사진 또는 실행취소·전체지우기·개체 목록) 둘레 배치. 전체 폭을 `max-w-5xl`로 제한해 페이지 max-w-[1600px]에서 지도가 838px 높이로 커져 도구가 한 화면에 안 담기던 문제를 해결했다(데스크탑 지도 약 772×434px). 캔버스 좌표는 boundingRect 비율로 정규화되므로 크기 축소가 드로잉 데이터에 영향 없음. 컨테이너 16:9(aspect-video)는 960×540 합성 좌표계와의 왜곡 방지를 위해 유지해야 한다.
- **검증 결과** — `npx eslint`(대상 파일), `npx tsc --noEmit`이 종료 코드 0으로 통과했다. 실기기 지도 고정·모바일 터치 드로잉은 수동 확인 항목.

## 2026-07-11 지도 고정 실원인 확정 — React DOM 재사용 (Playwright 검증)

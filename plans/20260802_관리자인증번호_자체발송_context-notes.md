...
## 2026-08-03 · 수신 이메일을 직접 입력받는다

`admin` 아이디만 알면 누구나 발송 버튼을 누를 수 있어 발송이 잦아질 수 있다는 사용자 지적. 아이디 확인 뒤 **수신 이메일 입력란**을 하나 더 두고, 서버가 `ADMIN_OTP_EMAIL`과 대소문자 무시 비교해서 다르면 403 `관리자 이메일이 아닙니다.`로 끊는다. 이 검사는 쿨다운·한도 검사보다 **앞**에 둬서, 틀린 주소로는 DB 쓰기도 메일 발송도 일어나지 않게 했다.

- 트레이드오프. 틀렸다고 알려주는 순간 관리자 메일 주소를 추측으로 찾아낼 통로가 열린다. 다만 주소를 알아내도 인증번호는 그 메일함으로만 가므로 로그인은 불가하고, 발송 남용 차단 효과가 더 크다고 보고 요청대로 명시적 문구를 유지했다.
- `verify/route.ts`는 손대지 않았다. 실제 비밀은 6자리 코드이고, 세션 계정은 여전히 서버 env(`ADMIN_OTP_EMAIL`)로만 정해진다 — 클라이언트가 보낸 주소는 세션 발급에 쓰이지 않는다.

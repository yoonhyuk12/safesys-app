...
- **사진 설명 기본값 (후속 요청)** — 검측 사항(inspection_items)을 줄 단위로 나눠 사진1 ← 1줄, 사진2 ← 2줄을 기본값으로. 업로드 시점 + 탭 진입 시(빈 설명만, goToChecklist의 일자 자동 채움과 동일 패턴) 두 곳에서 채운다. 사용자가 수정한 설명은 덮어쓰지 않는다.
- **크롭 버튼 무반응 제보 조사 결과** — 코드 문제 아님. 임시 페이지(/dev-photo-test, 검증 후 삭제)로 Playwright 재현 결과 모달 정상 오픈(스크린샷 확보). 유력 원인은 페이지에 주입된 agentation 피드백 위젯의 상호작용 차단(Block page interactions) 또는 이전 좀비 서버. 사용자 재확인 요청함.

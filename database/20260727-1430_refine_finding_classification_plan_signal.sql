-- 지적 분류 규칙 보정: 작업계획서→F16_DOC, 신호수·유도원·작업지휘→F07_SIGNAL override 추가

-- =============================================================================
-- 배경
--   '건설기계 작업계획서 미작성', '장비작업 시 신호수 미배치' 처럼 장비 키워드가 함께 있는 지적이
--   F05_MACHINERY(건설기계·중장비)로 흡수돼 서류·신호수 통계가 과소, F05가 과대 집계됐다.
--   F05 규칙이 F07·F16보다 앞 순서라 '장비'·'건설기계'·'차량계'가 먼저 매칭되기 때문이다.
--
-- 변경
--   exact override 블록(레미콘 박스 다음)에 두 규칙을 추가한다.
--     1) 작업계획서            → F16_DOC     (무조건)
--     2) 신호수|유도원|작업지휘 → F07_SIGNAL  (단 F01 보호구 키워드가 없을 때만)
--   보호구 예외는 '신호수 안전모 미착용', '작업지휘자 안전모 미착용' 처럼
--   주어만 신호수이고 실제 결함은 보호구인 지적을 F01_PPE에 남기기 위한 것이다.
--   두 규칙이 동시에 걸리면 작업계획서(F16)가 우선한다.
--
-- 적용 순서
--   1) 분류 함수 교체 (컬럼·트리거·CHECK 는 20260718-1120 그대로 유지)
--   2) 기존 행 재백필
-- 재실행 안전: OR REPLACE + 전체 재계산 UPDATE
-- Supabase 콘솔에서 수동 적용. TS 사본은 safesys-app/src/lib/finding-classification.ts
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. 분류 함수 교체 (TS finding-classification.ts 와 동일 순서)
-- -----------------------------------------------------------------------------
...
BEGIN
  ...
  IF t ~* '신호수|유도원|작업지휘'
     AND t !~* '안전모|안전대|안전화|안전띠|안전벨트|보호구|보호복|턱끈|턱근|안전복|용접[[:space:]]*안전|안전장화|구명조끼|구명환|구명줄'
  THEN
    RETURN 'F07_SIGNAL';
  END IF;
...
COMMENT ON FUNCTION public.classify_inspection_finding(TEXT) IS
  '점검 지적 원문을 F01_PPE~F20_WORK_METHOD로 분류한다. 빈 값·메타/상태 문구(작업없음·해당사항 없음·공사 준비/중지 등, DEFECT 없을 때)는 NULL. 작업계획서는 F16, 신호수·유도원·작업지휘는 F07(보호구 키워드 동반 시 F01 유지). F20 표시명은 작업방법 미준수.';

-- -----------------------------------------------------------------------------
-- 2. 기존 데이터 재백필 (정기: findings가 비면 field_item fallback)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. 적용 확인 (선택) — 이동 결과를 눈으로 확인할 때 사용
-- -----------------------------------------------------------------------------
-- SELECT finding_category_code, COUNT(*)
-- FROM public.safety_inspection_results
-- WHERE finding_category_code IS NOT NULL

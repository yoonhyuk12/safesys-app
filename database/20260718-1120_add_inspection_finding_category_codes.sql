-- 정기·본부불시점검 지적 원문을 F01~F20 분류코드로 자동 저장하는 컬럼·함수·트리거·백필 마이그레이션

-- =============================================================================
-- 적용 순서 (기존 데이터 안전)
--   1) 분류 컬럼 추가 (NULL 허용, CHECK 없음)
--   2) 분류 함수 생성/교체
--   3) 기존 행 백필 (함수로 코드 계산)
--   4) INSERT/UPDATE 트리거 연결
--   5) F01~F20 유효성 CHECK 추가
-- 재실행 안전: IF NOT EXISTS / OR REPLACE / DROP IF EXISTS 사용
-- Supabase 콘솔에서 수동 적용. 이 파일만으로 완료되며 MCP 적용 대상 아님.
-- =============================================================================
...
ALTER TABLE public.safety_inspection_results
  ADD COLUMN IF NOT EXISTS finding_category_code TEXT;

ALTER TABLE public.headquarters_inspections
  ADD COLUMN IF NOT EXISTS issue1_category_code TEXT,
  ADD COLUMN IF NOT EXISTS issue2_category_code TEXT;

COMMENT ON COLUMN public.safety_inspection_results.finding_category_code IS
  '지적사항 고정 분류코드 (F01_PPE~F20_WORK_METHOD). findings가 비면 field_item으로 자동 계산. 메타·무의미 문구는 NULL';

COMMENT ON COLUMN public.headquarters_inspections.issue1_category_code IS
  '지적1(issue_content1) 고정 분류코드. 원문 기준 자동 계산. 메타·무의미 문구는 NULL';

COMMENT ON COLUMN public.headquarters_inspections.issue2_category_code IS
  '지적2(issue_content2) 고정 분류코드. 원문이 비어 있으면 NULL. F20_WORK_METHOD 표시명은 작업방법 미준수';

-- -----------------------------------------------------------------------------
-- 2. 분류 함수 (TS finding-classification.ts 와 동일 순서)
--    제외 → exact override → F01→F18→F20 → F19_OTHER
--    DEFECT 신호(세 분기 공통): 미착용|미설치|미배치|미흡|불량|누락|미확보|부적정|필요|교체|지시|위험|미사용|미준수|미비
--    사용자 확정 재분류 (F19 91건: NULL 29 / 재분류 60 / CCTV F19 2)
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 3. 기존 데이터 백필 (트리거 전에 수행해 적용 과정을 명확히 함)
--    정기: findings가 비면 field_item fallback
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 4. 트리거: 신규 입력·원문 수정 시 항상 재계산
--    정기: findings 또는 field_item 변경 시 동일 fallback으로 재계산
-- -----------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.set_safety_inspection_result_finding_category()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'INSERT'
     ...
     OR NEW.field_item IS DISTINCT FROM OLD.field_item
  THEN
    NEW.finding_category_code := public.classify_inspection_finding(
      COALESCE(NULLIF(BTRIM(NEW.findings), ''), NEW.field_item)
    );
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS trg_safety_inspection_results_finding_category
  ON public.safety_inspection_results;

CREATE TRIGGER trg_safety_inspection_results_finding_category
  BEFORE INSERT OR UPDATE ON public.safety_inspection_results
  FOR EACH ROW
  EXECUTE FUNCTION public.set_safety_inspection_result_finding_category();

CREATE OR REPLACE FUNCTION public.set_headquarters_inspection_issue_categories()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
  IF TG_OP = 'INSERT'
     OR NEW.issue_content1 IS DISTINCT FROM OLD.issue_content1
  THEN
    NEW.issue1_category_code := public.classify_inspection_finding(NEW.issue_content1);
  END IF;

  IF TG_OP = 'INSERT'
     OR NEW.issue_content2 IS DISTINCT FROM OLD.issue_content2
  THEN
    -- issue2가 비어 있으면 함수가 NULL 반환
    NEW.issue2_category_code := public.classify_inspection_finding(NEW.issue_content2);
  END IF;
...
DROP TRIGGER IF EXISTS trg_headquarters_inspections_issue_categories
  ON public.headquarters_inspections;

CREATE TRIGGER trg_headquarters_inspections_issue_categories
  BEFORE INSERT OR UPDATE ON public.headquarters_inspections
  FOR EACH ROW
  EXECUTE FUNCTION public.set_headquarters_inspection_issue_categories();

-- -----------------------------------------------------------------------------
-- 5. F01~F20 유효성 CHECK (NULL 허용)
-- -----------------------------------------------------------------------------
ALTER TABLE public.safety_inspection_results
  DROP CONSTRAINT IF EXISTS safety_inspection_results_finding_category_code_check;

ALTER TABLE public.safety_inspection_results
  ADD CONSTRAINT safety_inspection_results_finding_category_code_check
  CHECK (
    finding_category_code IS NULL
    OR finding_category_code IN (
      'F01_PPE', 'F02_FALL', 'F03_ACCESS', 'F04_SCAFFOLD', 'F05_MACHINERY',
      'F06_LIFTING', 'F07_SIGNAL', 'F08_ACCESS_CTRL', 'F09_SIGNAGE', 'F10_HOUSEKEEP',
      'F11_MATERIAL', 'F12_FLOOD', 'F13_ELEC', 'F14_TOOL', 'F15_HAZMAT',
      'F16_DOC', 'F17_WELFARE', 'F18_QUALITY', 'F19_OTHER', 'F20_WORK_METHOD'
    )
  );

ALTER TABLE public.headquarters_inspections
  DROP CONSTRAINT IF EXISTS headquarters_inspections_issue1_category_code_check;

ALTER TABLE public.headquarters_inspections
  ADD CONSTRAINT headquarters_inspections_issue1_category_code_check
  CHECK (
    issue1_category_code IS NULL
    OR issue1_category_code IN (
      'F01_PPE', 'F02_FALL', 'F03_ACCESS', 'F04_SCAFFOLD', 'F05_MACHINERY',
      'F06_LIFTING', 'F07_SIGNAL', 'F08_ACCESS_CTRL', 'F09_SIGNAGE', 'F10_HOUSEKEEP',
      'F11_MATERIAL', 'F12_FLOOD', 'F13_ELEC', 'F14_TOOL', 'F15_HAZMAT',
      'F16_DOC', 'F17_WELFARE', 'F18_QUALITY', 'F19_OTHER', 'F20_WORK_METHOD'
...
ALTER TABLE public.headquarters_inspections
  DROP CONSTRAINT IF EXISTS headquarters_inspections_issue2_category_code_check;

ALTER TABLE public.headquarters_inspections
  ADD CONSTRAINT headquarters_inspections_issue2_category_code_check
  CHECK (
    issue2_category_code IS NULL
    OR issue2_category_code IN (
      'F01_PPE', 'F02_FALL', 'F03_ACCESS', 'F04_SCAFFOLD', 'F05_MACHINERY',
      'F06_LIFTING', 'F07_SIGNAL', 'F08_ACCESS_CTRL', 'F09_SIGNAGE', 'F10_HOUSEKEEP',
      'F11_MATERIAL', 'F12_FLOOD', 'F13_ELEC', 'F14_TOOL', 'F15_HAZMAT',
      'F16_DOC', 'F17_WELFARE', 'F18_QUALITY', 'F19_OTHER', 'F20_WORK_METHOD'
...
-- -----------------------------------------------------------------------------
-- 정적 검토 메모 (적용 후 권장 검증 쿼리, 실행 안 함)
--   SELECT finding_category_code, COUNT(*) FROM safety_inspection_results GROUP BY 1;
--   SELECT issue1_category_code, COUNT(*) FROM headquarters_inspections GROUP BY 1;
...
--   SELECT public.classify_inspection_finding('작업 종료 후 안전시설물 등으로 통행인 넘어짐 사고 등 방지조치 필요'); -- F10_HOUSEKEEP
--   SELECT public.classify_inspection_finding('돌출부위 주변 넘어짐 위험 발생 가능');     -- F10_HOUSEKEEP
--   SELECT public.classify_inspection_finding(NULL);                                     -- NULL
--   SELECT public.classify_inspection_finding('');                                       -- NULL
--   정기 fallback: findings 빈 값 + field_item 유의미 텍스트 → field_item 분류
--
--   TS/SQL 패리티 후보 6 + 일반 동시진행 음성 샘플
--   SELECT public.classify_inspection_finding('그라인더 덮개 전부 미사용');               -- F14_TOOL (덮개 F11보다 선행)
--   SELECT public.classify_inspection_finding('장비 구름 방지 장치 조치 미흡');           -- F06_LIFTING (장비 F05보다 선행)
--   SELECT public.classify_inspection_finding('보안경 미착용');                          -- F14_TOOL
--   SELECT public.classify_inspection_finding('안전 시설 설치 미흡');                    -- F02_FALL (공백형 안전 시설)
--   SELECT public.classify_inspection_finding('근로자 작업가능상태 점검표 작성 필요');   -- F16_DOC
--   SELECT public.classify_inspection_finding('작업 방법 미준수');                       -- F20_WORK_METHOD
--   SELECT public.classify_inspection_finding('동시 진행으로 인한 간섭 주의');           -- NOT F20 (단독 동시진행 음성 → 타코드/F19)
--
--   상태 메타 NULL 샘플 (전국 F19 잔류 10변형)
...
--   SELECT public.classify_inspection_finding('작업 없음 구간 난간 미설치');             -- F02_FALL (DEFECT 있으면 유지)
--
--   적용 후 기대치 (경기·기간 2026-01-01~2026-07-18 사고분석 UI 규칙)
--   F19 = CCTV 2건만 (이동형cctv…, Cctv 공사현장…). 메타 상태문구는 NULL로 집계 제외
-- =============================================================================

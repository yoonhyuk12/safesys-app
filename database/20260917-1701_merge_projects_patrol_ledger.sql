-- 프로젝트 병합에 순회점검대장을 포함하고 FK 자식 개수 가드를 29개로 갱신한다.
-- 적용 순서: 20260917-1700_순회점검대장.sql 다음에 이 파일을 실행한다.
-- 기존 장비 대장 병합 함수의 데이터 보존·충돌 차단·권한 정책은 유지한다.
...
BEGIN
  ...
  IF v_fk_count <> 29 THEN
    RAISE EXCEPTION 'merge_projects가 아는 자식 테이블(29개)과 실제 FK 테이블 수(%개)가 다릅니다. 함수를 갱신하세요.', v_fk_count;
  END IF;
  ...
  UPDATE patrol_ledger_inspections      SET project_id = p_target WHERE project_id = p_source;
...
COMMENT ON FUNCTION public.merge_projects(UUID, UUID) IS
  'source 프로젝트의 자식 행 29종과 비발주청 공유자를 target으로 이전하고, 비어 있는 선택값과 주소·계약 묶음을 보충한 뒤 source를 삭제한다. 두 프로젝트 행을 id 순서로 잠근 뒤 preview_project_merge_v2로 최종 확인하며, 그 응답이 세 필드 형식이 아니면 판정을 신뢰할 수 없으므로 병합을 중단한다. 작업일보·품질 월간보고서가 하나라도 겹치면 MERGE_REPORT_CONFLICT로, source 공정표를 원본 그대로 옮길 수 없으면 MERGE_SCHEDULE_CONFLICT로 전체를 롤백한다. 실제 FK 자식 테이블 수와 다르면 유실 방지를 위해 중단하며 service-role에서만 호출한다.';

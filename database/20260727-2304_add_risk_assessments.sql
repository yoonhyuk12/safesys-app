-- 수시 위험성평가서 저장소와 프로젝트 확정 사업별 컬럼, 프로젝트 병합 처리를 추가하는 마이그레이션

-- 프로젝트별로 한 번 확정한 사업별(위험요인 DB 16종)을 기억해 다음 평가서에서 기본값으로 쓴다.
-- Worker A의 참조 테이블 마이그레이션과 겹칠 수 있어 IF NOT EXISTS로 멱등 처리한다.
ALTER TABLE public.projects ADD COLUMN IF NOT EXISTS risk_business_type TEXT;

COMMENT ON COLUMN public.projects.risk_business_type IS
  '위험요인 DB의 확정 사업별(16종). NULL이면 미확정 또는 사업 무관(전체) 모드';

CREATE TABLE public.risk_assessments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  assessment_type TEXT NOT NULL DEFAULT '수시' CHECK (assessment_type IN ('수시')),
  title TEXT NOT NULL CHECK (BTRIM(title) <> ''),
  business_type TEXT,
  "trigger" TEXT NOT NULL DEFAULT '',
  author_name TEXT NOT NULL DEFAULT '',
  manage_period_start DATE,
  manage_period_end DATE,
...
CREATE INDEX idx_risk_assessments_project_created_at
...
-- 한 평가서를 여러 담당이 이어서 보완하므로 로그인 사용자의 공동 수정·삭제를 허용한다.
...
CREATE OR REPLACE FUNCTION public.update_risk_assessments_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
...
CREATE TRIGGER risk_assessments_updated_at_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION public.update_risk_assessments_updated_at();

COMMENT ON TABLE public.risk_assessments IS '프로젝트별 수시 위험성평가서 1건(표 행·결재 서명은 JSONB)';
COMMENT ON COLUMN public.risk_assessments."trigger" IS '수시평가 사유(작업 변경, 재해 발생 등 자유 텍스트)';
COMMENT ON COLUMN public.risk_assessments."rows" IS 'RiskAssessmentRow[] — 양식 2행 1조가 배열 원소 1개';
COMMENT ON COLUMN public.risk_assessments.signatures IS 'RiskAssessmentSignatures — 공사/안전/현장소장/공사감독 서명 dataURL';

-- merge_projects 갱신: 자식 테이블 26개 (2026-07-27 risk_assessments 추가)
...
BEGIN
  ...
  IF v_fk_count <> 26 THEN
    RAISE EXCEPTION 'merge_projects가 아는 자식 테이블(26개)과 실제 FK 테이블 수(%개)가 다릅니다. 함수를 갱신하세요.', v_fk_count;
  END IF;
  ...
  UPDATE projects AS tgt
     SET construction_start_date = COALESCE(tgt.construction_start_date, src.construction_start_date),
         ...
         risk_business_type = CASE
           WHEN (tgt.risk_business_type IS NULL OR BTRIM(tgt.risk_business_type) = '')
             AND src.risk_business_type IS NOT NULL AND BTRIM(src.risk_business_type) <> ''
           THEN src.risk_business_type ELSE tgt.risk_business_type END,
         total_budget = CASE
...
COMMENT ON FUNCTION public.merge_projects(UUID, UUID) IS
  'source 프로젝트의 자식 행 26종과 비발주청 공유자를 target으로 이전하고, 비발주청 source 소유자를 target 공유자로 보존하며, 빈 선택사항과 분기·준공 상태를 보충한 후 source를 삭제한다. 실제 FK 자식 테이블 수와 다르면 유실 방지를 위해 중단하며 service-role에서만 호출한다.';

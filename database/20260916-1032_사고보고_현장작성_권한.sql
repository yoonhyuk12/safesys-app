...
--     옮기거나 남의 사고를 가져오는 것을 막는다.
--   * created_by는 INSERT WITH CHECK의 auth.uid() 일치와 기존
--     project_accidents_created_by_immutable_trigger가 함께 지킨다.
...
-- 수정은 작성자 본인이면서 그 현장을 여전히 볼 수 있을 때만 한다.
CREATE POLICY "작성자 사고 수정"
...
-- 삭제도 수정과 같은 기준이다. 잘못 올린 보고는 올린 사람이 거둔다.
...
COMMENT ON TABLE public.project_accidents IS
  '프로젝트별 실제 사고와 피해·원인·예방조치 이력. 등록 현장 사고(project_id)는 그 현장을 볼 수 있는 사용자(시공사·감리단·관할 발주청)가 작성하고 작성자 본인 또는 관할 본부급 이상 발주청이 고치거나 지운다. 외부 미등록 현장 사고(project_id NULL + external_project_name)는 관할 발주청이 조회하고 본부급 이상 발주청이 등록·수정·삭제한다.';

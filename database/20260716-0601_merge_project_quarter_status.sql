-- 프로젝트 병합 시 빈 선택사항과 분기·준공 상태를 흡수될 프로젝트 정보로 보충한다.
...
COMMENT ON FUNCTION public.merge_projects(UUID, UUID) IS
  'source 프로젝트의 자식 행 24종을 target으로 이전하고 target의 빈 선택사항과 비어 있는 분기·준공 상태를 source 값으로 보충한 후 source를 삭제한다. 실제 FK 자식 테이블 수와 다르면 유실 방지를 위해 중단하며 service-role에서만 호출한다.';

-- 프로젝트 사고 이력 저장소와 조직 관할 권한, 프로젝트 병합 처리를 추가하는 마이그레이션

CREATE TABLE public.project_accidents (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  project_id UUID NOT NULL REFERENCES public.projects(id) ON DELETE CASCADE,
  accident_at TIMESTAMPTZ NOT NULL,
  severity TEXT NOT NULL CHECK (severity IN ('minor', 'lost_time', 'serious', 'fatal')),
  accident_type TEXT NOT NULL CHECK (BTRIM(accident_type) <> ''),
  location TEXT NOT NULL CHECK (BTRIM(location) <> ''),
  work_description TEXT NOT NULL CHECK (BTRIM(work_description) <> ''),
  description TEXT NOT NULL CHECK (BTRIM(description) <> ''),
  cause TEXT NOT NULL CHECK (BTRIM(cause) <> ''),
  prevention_action TEXT NOT NULL CHECK (BTRIM(prevention_action) <> ''),
  injured_count INTEGER NOT NULL DEFAULT 0 CHECK (injured_count >= 0),
  fatal_count INTEGER NOT NULL DEFAULT 0 CHECK (fatal_count >= 0),
  lost_workdays INTEGER NOT NULL DEFAULT 0 CHECK (lost_workdays >= 0),
  created_by UUID REFERENCES auth.users(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

CREATE INDEX idx_project_accidents_project_accident_at
  ON public.project_accidents(project_id, accident_at DESC);
CREATE INDEX idx_project_accidents_accident_at
  ON public.project_accidents(accident_at DESC);
CREATE INDEX idx_project_accidents_severity
  ON public.project_accidents(severity);
CREATE INDEX idx_project_accidents_accident_type
  ON public.project_accidents(accident_type);

CREATE OR REPLACE FUNCTION public.update_project_accidents_updated_at()
RETURNS TRIGGER
LANGUAGE plpgsql
...
CREATE TRIGGER project_accidents_updated_at_trigger
  BEFORE UPDATE ON public.project_accidents
  FOR EACH ROW
  EXECUTE FUNCTION public.update_project_accidents_updated_at();

CREATE OR REPLACE FUNCTION public.prevent_project_accidents_created_by_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SET search_path = public
AS $$
...
CREATE TRIGGER project_accidents_created_by_immutable_trigger
  BEFORE UPDATE ON public.project_accidents
  FOR EACH ROW
  EXECUTE FUNCTION public.prevent_project_accidents_created_by_change();

ALTER TABLE public.project_accidents ENABLE ROW LEVEL SECURITY;

-- 모든 발주청 사용자는 본사·본부·지사 조직 계층에 따른 관할 사고를 조회한다.
  ...
  USING (
    EXISTS (
      SELECT 1
        ...
        JOIN public.projects p ON p.id = project_accidents.project_id
       WHERE up.id = auth.uid()
         AND up.role = '발주청'
         AND (
           up.hq_division IS NULL
           OR (up.hq_division = '본사' AND up.branch_division = '본사')
           OR (up.branch_division LIKE '%본부' AND p.managing_hq = up.hq_division)
           OR (
...
-- 본사·관리자급은 전사, 본부 사용자는 소속 본부 프로젝트에만 사고를 등록한다.
...
COMMENT ON TABLE public.project_accidents IS '프로젝트별 실제 사고와 피해·원인·예방조치 이력';
COMMENT ON COLUMN public.project_accidents.severity IS '사고 중대도 코드: minor=경상, lost_time=휴업, serious=중상, fatal=사망';
...
BEGIN
  ...
  IF v_fk_count <> 25 THEN
    RAISE EXCEPTION 'merge_projects가 아는 자식 테이블(25개)과 실제 FK 테이블 수(%개)가 다릅니다. 함수를 갱신하세요.', v_fk_count;
  END IF;
...
COMMENT ON FUNCTION public.merge_projects(UUID, UUID) IS
  'source 프로젝트의 자식 행 25종과 비발주청 공유자를 target으로 이전하고, 비발주청 source 소유자를 target 공유자로 보존하며, 빈 선택사항과 분기·준공 상태를 보충한 후 source를 삭제한다. 실제 FK 자식 테이블 수와 다르면 유실 방지를 위해 중단하며 service-role에서만 호출한다.';

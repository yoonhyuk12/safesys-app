-- 프로젝트 병합 함수(merge_projects)에 ai_usage_logs를 반영해 FK 가드(26→27) 불일치로 인한 병합 실패를 해소한다
-- 배경: 2026-08-28 ai_usage_logs(project_id → projects, ON DELETE SET NULL) 추가로 실제 FK 자식 테이블이 27개가 되어
--       가드가 예외를 던지며 /api/projects/merge가 500을 반환했다.
...
COMMENT ON FUNCTION public.merge_projects(UUID, UUID) IS
  'source 프로젝트의 자식 행 27종과 비발주청 공유자를 target으로 이전하고, 비발주청 source 소유자를 target 공유자로 보존하며, 빈 선택사항과 분기·준공 상태를 보충한 후 source를 삭제한다. 실제 FK 자식 테이블 수와 다르면 유실 방지를 위해 중단하며 service-role에서만 호출한다.';
